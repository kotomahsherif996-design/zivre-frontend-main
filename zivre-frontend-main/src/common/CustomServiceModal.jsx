...
import ScheduleSelector, { defaultSchedule } from './ScheduleSelector'
    ...
    try {
      if (schedule.schedule_type === 'one_time') {
        ...
        await createSchedule({
          ...
          end_date: schedule.end_mode === 'date' && schedule.end_date ? schedule.end_date : null
        })
      }
      onSubmitSuccess()
      onClose()
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      ...
      <DialogActions>
        ...
        {activeStep === 2 && <Button variant="contained" onClick={handleSubmit} disabled={submitting} sx={{ bgcolor: '#10b981' }}>{submitting ? <CircularProgress size={24} /> : (schedule.schedule_type === 'one_time' ? 'Submit Request' : 'Create Schedule')}</Button>}
      </DialogActions>
    </Dialog>
  )
}
