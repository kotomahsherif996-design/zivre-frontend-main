...
const DemoTour = ({ open, onClose, onComplete, steps, title = "Guided Tour" }) => {
  ...
  return (
    ...
    >
      <Box sx={{ background: 'linear-gradient(135deg, #0a1f1a 0%, #0f3b2c 70%, #10b981 180%)', color: 'white', p: 2.5, display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'relative', overflow: 'hidden' }}>
        <Box sx={{ position: 'absolute', top: -30, right: 40, width: 120, height: 120, borderRadius: '50%', background: 'radial-gradient(circle, rgba(16,185,129,0.4), transparent 70%)', pointerEvents: 'none' }} />
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.2, position: 'relative' }}>
          <SchoolIcon />
          <Typography variant="h6" sx={{ fontFamily: '"Sora","Inter",sans-serif', fontWeight: 700 }}>{title}</Typography>
        </Box>
        <IconButton onClick={handleSkip} sx={{ color: 'white', position: 'relative' }}>
          <CloseIcon />
        </IconButton>
      </Box>
      ...
      <DialogActions sx={{ p: 2, borderTop: '1px solid #e9efec' }}>
        ...
        <Button 
          ...
          sx={{ bgcolor: '#10b981', '&:hover': { bgcolor: '#0f3b2c' } }}
        >
