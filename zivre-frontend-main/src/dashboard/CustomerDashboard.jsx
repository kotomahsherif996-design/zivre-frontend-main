...
import ScheduleSelector, { defaultSchedule } from '../common/ScheduleSelector'
...
// Fallback polling interval (5 seconds for faster updates)
  ...
  const handleRequest = async () => {
    ...
    try {
      ...
      let response
      if (requestSchedule.schedule_type === 'one_time') {
        ...
        response = await createSchedule({
          ...
          end_date: requestSchedule.end_mode === 'date' && requestSchedule.end_date ? requestSchedule.end_date : null
        })
        showToast('Recurring service scheduled successfully!', 'success')
        ...
        <Box component="nav" sx={{ width: { md: drawerWidth }, flexShrink: { md: 0 } }}>
          ...
          <Dialog open={openRequestModal} onClose={() => setOpenRequestModal(false)} maxWidth="sm" fullWidth slotProps={{ paper: { sx: { borderRadius: 3, maxHeight: '90vh' } } }}>
            ...
            <DialogContent>
              ...
              />
              <ScheduleSelector value={requestSchedule} onChange={setRequestSchedule} />
            </DialogContent>
            <DialogActions>
              ...
              <Button
                ...
                sx={{ bgcolor: '#10b981', '&:hover': { bgcolor: '#059669' } }}
              >
                {actionLoading === 'request' ? <CircularProgress size={24} sx={{ color: 'white' }} /> : (requestSchedule.schedule_type === 'one_time' ? 'Submit Request' : 'Create Schedule')}
              </Button>
            </DialogActions>
          </Dialog>
          {isMobile && (
