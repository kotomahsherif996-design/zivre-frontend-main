...
import {
  getAdminStats, getServices, toggleServiceActive, updateService, createService, deleteService, getAllSchedules,
...
const AdminDashboard = () => {
  ...
  const loadData = useCallback(async (showSpinner = true) => {
    ...
    try {
      setLoading(false)
      const [statsRes, servicesRes, quotesRes, requestsRes, usersRes, commentsRes, schedulesRes] = await Promise.all([
        getAdminStats(),
        getServices(false),
        getQuotes(),
        getAllRequests(),
  ...
  const filteredQuotes = useMemo(() => {
    ...
    if (searchTerm && searchTerm.trim() && activeTab === 3) {
      const term = searchTerm.toLowerCase().trim()
      filtered = filtered.filter(q => q.full_name?.toLowerCase().includes(term) ||
  ...
  const filteredComments = useMemo(() => {
    ...
    if (searchTerm && searchTerm.trim() && activeTab === 6) {
      const term = searchTerm.toLowerCase().trim()
      filtered = filtered.filter(c => c.user_name?.toLowerCase().includes(term) ||
  ...
  const assignableRequests = useMemo(() => requests.filter(r => r.status === 'pending_approval'),
    [requests]
  )

  const assignedRequests = useMemo(() => requests.filter(r => r.status === 'assigned' || r.status === 'in_progress'),
    [requests]
  )
...
const handleDeleteRequestPermanently = async (requestId) => {
  ...
  const handleNotifyCustomer = async (requestId) => {
    setActionLoading('notify_' + requestId)
    try {
  ...
  const handleDeleteUser = async (userId, userName) => {
    setActionLoading('deluser_' + userId)
    try {
  ...
  const handleUpdateQuoteStatus = async (quoteId, status) => {
    setActionLoading('quote_' + quoteId)
    try {
  ...
  const handleDeleteService = async (serviceId, serviceName) => {
    ...
    setActionLoading('delsvc_' + serviceId)
    try {
      await deleteService(serviceId)
      showToast('Service deleted', 'success')
      loadData(false)
    } catch (err) {
      showToast(err.response?.data?.error || 'Could not delete service', 'error')
    } finally {
      setActionLoading(null)
    }
  }
  ...
  const menuItems = [
    ...
    { label: 'Scheduled Services', icon: <EventIcon />, tab: 12, badge: schedules.filter(s => s.status === 'active').length },
  ...
  return (
    <>
      ...
      <Box sx={{ display: 'flex', minHeight: 'calc(100vh - 64px)', bgcolor: '#f4f8f6' }}>
        ...
        <Box component="main" sx={{ flexGrow: 1, p: { xs: 2, md: 3 }, width: { md: `calc(100% - ${drawerWidth}px)` } }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3, flexWrap: 'wrap', gap: 2 }}>
            <Typography variant="h4" sx={{ fontFamily: '"Sora","Inter",sans-serif', fontWeight: 800, color: '#0a1f1a' }}>Admin Dashboard</Typography>
          ...
          {activeTab === 0 && (
            <>
              ...
              <Card sx={{ p: 3, mb: 3, bgcolor: '#f4f8f6' }}>
                      ...
                      <TableRow key={s.id} sx={{ '&:hover': { bgcolor: '#f8fafc' } }}>
                        ...
                        <TableCell>
                          ...
                          <Tooltip title="Delete service">
                            <IconButton size="small" onClick={() => handleDeleteService(s.id, s.name)} disabled={actionLoading === 'delsvc_' + s.id} sx={{ color: '#ef4444' }}>
                              {actionLoading === 'delsvc_' + s.id ? <CircularProgress size={16} /> : <DeleteIcon fontSize="small" />}
                            </IconButton>
                          </Tooltip>
                        </TableCell>
                      </TableRow>
                  ...
                  <Card key={r.id} sx={{ p: 2, mb: 2, border: '1px solid #e2e8f0', bgcolor: '#fef3c7' }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 2 }}>
                      <Box>
                        ...
                        <Typography variant="body2">Amount: <strong style={{ color: '#10b981' }}>GH₵{r.amount}</strong>{(r.period_days || 1) > 1 && <span style={{ color: '#64748b', fontWeight: 400 }}> &nbsp;({r.period_days} days)</span>}</Typography>
                          ...
                          <Box sx={{ mt: 1, pl: 2, bgcolor: '#fff7ed', borderRadius: 1, p: 1 }}>
                            ...
                            {r.components_data.map((comp, idx) => (
                              <Box key={idx} component="div" sx={{ display: 'block', fontSize: '0.7rem', mt: 0.5 }}>
                                {comp.quantity} × {comp.name} {comp.period_days > 1 ? `@ GHS${comp.daily_unit_price}/day × ${comp.period_days}d` : `@ GHS${comp.unit_price}`} = GHS{comp.subtotal}
                              </Box>
                            ))}
                          </Box>
                        )}
          ...
          {activeTab === 7 && (
            <Card sx={{ p: 3 }}>
              ...
              <Button
                ...
                disabled={actionLoading === 'percentages' || Math.abs(percentages.provider_percent + percentages.admin_percent + percentages.site_fee_percent + (percentages.referral_pool_percent || 0) - 100) > 0.01}
                sx={{ mt: 3, bgcolor: '#10b981' }}
              >
                {actionLoading === 'percentages' ? <CircularProgress size={24} sx={{ color: 'white' }} /> : 'Update Percentages'}
              </Button>
            </Card>
          )}
                    ...
                    <TableBody>
                      {filteredSchedules.map((s) => {
                        ...
                        const freq = s.schedule_type === 'daily' ? 'Every day'
                          ...
                          : s.schedule_type === 'monthly' ? `Monthly (day ${s.monthly_date})`
                                ...
                                <>
                                  ...
                                  {periodDays > 1 && (
                                    <Typography variant="caption" color="text.secondary" sx={{ display: 'block' }}>
                                      GH₵{s.per_day_amount.toFixed(2)} × {periodDays}
                                    </Typography>
                                  )}
                                </>
                              ) : '—'}
          ...
          <Dialog open={openServiceModal} onClose={() => { setOpenServiceModal(false); setEditingService(null) }} maxWidth="sm" fullWidth>
            ...
            <DialogActions>
              ...
              <Button
                ...
                sx={{ bgcolor: '#10b981', '&:hover': { bgcolor: '#059669' } }}
              >
                {actionLoading === 'service' ? <CircularProgress size={24} sx={{ color: 'white' }} /> : (editingService ? 'Save Changes' : 'Create Service')}
              </Button>
            </DialogActions>
          </Dialog>
      ...
      <ConfirmModal
        ...
        title="Delete User"
        message={`Are you sure you want to delete "${userToDelete?.full_name || 'this user'}"?\n\nThis will permanently remove:\n• All service requests\n• All messages\n• All notifications\n• All comments\n\nThis action CANNOT be undone!`}
