...
const ProviderDashboard = () => {
  ...
  const filteredAvailableJobs = useMemo(() => {
    ...
    return availableJobs.filter(job => job.service_name?.toLowerCase().includes(term) ||
  ...
  const filteredMyJobs = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      const term = searchTerm.toLowerCase().trim()
      filtered = filtered.filter(job => job.service_name?.toLowerCase().includes(term) ||
        job.customer_name?.toLowerCase().includes(term) ||
        job.location_city?.toLowerCase().includes(term) ||
        job.location_region?.toLowerCase().includes(term)
      )
  ...
  return (
    <>
      ...
      <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: '#f4f8f6' }}>
        ...
        <Box component="main" sx={{ flexGrow: 1, p: { xs: 2, md: 3 }, pb: { xs: 10, md: 3 }, width: { md: `calc(100% - ${drawerWidth}px)` } }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3, flexWrap: 'wrap', gap: 2 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              <Typography variant="h4" sx={{ fontFamily: '"Sora","Inter",sans-serif', fontWeight: 800, color: '#0a1f1a' }}>Provider Dashboard</Typography>
            </Box>
          ...
          <Card sx={{ p: 3, mb: 4, bgcolor: '#10b981', color: 'white', borderRadius: 3 }}>
            ...
            <Alert severity="info" sx={{ mt: 2, bgcolor: 'rgba(255,255,255,0.2)', color: 'white' }}>
               Customers will pay you only through our designated via MoMoPay number.
              <br />
                      ...
                      <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column', transition: 'transform 0.2s', '&:hover': { transform: 'translateY(-4px)', boxShadow: 3 } }}>
                        <CardContent sx={{ flexGrow: 1 }}>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
                            ...
                            {job.is_recurring && <Chip label="Recurring" size="small" sx={{ bgcolor: '#ede9fe', color: '#7c3aed', fontWeight: 600 }} />}
                          </Box>
                          ...
                          <Typography variant="h5" fontWeight="700" sx={{ color: '#10b981', mt: 2 }}>
                            GH₵{job.amount}
                            {(job.period_days || 1) > 1 && (
                              <Typography component="span" variant="caption" sx={{ color: '#64748b', fontWeight: 400, ml: 1 }}>
                                ({job.period_days} days)
                              </Typography>
                            )}
                          </Typography>
