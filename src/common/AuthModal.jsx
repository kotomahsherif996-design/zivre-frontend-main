...
const AuthModal = ({ isSignUp, role, onClose, onSuccess, onSwitchToSignIn, onSwitchToSignUp }) => {
  ...
  return (
    <>
      ...
      >
        <Box sx={{ background: 'linear-gradient(135deg, #0a1f1a 0%, #0f3b2c 70%, #10b981 180%)', color: 'white', px: 4, pt: 4, pb: 3, textAlign: 'center', position: 'relative' }}>
          <Box sx={{ position: 'absolute', top: -30, right: -20, width: 140, height: 140, borderRadius: '50%', background: 'radial-gradient(circle, rgba(16,185,129,0.4), transparent 70%)', pointerEvents: 'none' }} />
          <Typography sx={{ fontFamily: '"Sora","Inter",sans-serif', fontWeight: 800, letterSpacing: '0.12em', fontSize: '1.1rem', position: 'relative' }}>
            ZIVRE
          </Typography>
          <Typography variant="h6" component="div" sx={{ fontFamily: '"Sora","Inter",sans-serif', fontWeight: 800, mt: 1.5, position: 'relative' }}>
            {isSignUp ? 'Create your account' : 'Welcome back'}
          </Typography>
          <Typography variant="body2" sx={{ mt: 0.5, opacity: 0.82, position: 'relative' }}>
            {isSignUp
