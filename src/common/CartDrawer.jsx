                  ...
                  <Box sx={{ width: '100%', mb: 1, pl: 2 }}>
                    ...
                    {req.components_data.map((comp, idx) => (
                      <Typography key={idx} variant="body2" sx={{ fontSize: '0.8rem' }}>
                        ...
                        {comp.period_days > 1
                          ? ` (GHS${comp.daily_unit_price}/day × ${comp.period_days} days)`
                          : ` (GHS${comp.unit_price})`} = GHS{comp.subtotal}
                      </Typography>
                    ))}
                  </Box>
                )}
                {(req.period_days || 1) > 1 && (
                  <Box sx={{ width: '100%', mb: 1, pl: 2 }}>
                    <Typography variant="caption" sx={{ color: '#059669', fontWeight: 600 }}>
                      Covers {req.period_days} days (GHS{(req.amount / req.period_days).toFixed(2)}/day)
                    </Typography>
                  </Box>
                )}
