...
const NotificationDropdown = () => {
  ...
  useEffect(() => {
    fetchNotifications()
    // Realtime: refresh instantly when a notification arrives via WebSocket
    const handleRealtimeNotification = () => fetchNotifications()
    window.addEventListener('new_notification', handleRealtimeNotification)
    // Fallback poll (in case the socket drops)
    const interval = setInterval(fetchNotifications, 60000)
    return () => {
      window.removeEventListener('new_notification', handleRealtimeNotification)
  ...
  const getNotificationIcon = (type) => {
    const map = {
      ...
      job: { Icon: BuildIcon, color: '#3b82f6' },
      referral: { Icon: PaymentsIcon, color: '#10b981' },
      message: { Icon: ChatBubbleOutlineIcon, color: '#8b5cf6' },
    }
    const { Icon, color } = map[type] || { Icon: CampaignIcon, color: '#64748b' }
    return <Icon sx={{ color, fontSize: 22 }} />
  }
